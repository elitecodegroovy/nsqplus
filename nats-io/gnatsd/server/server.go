...
//Builds the server info with json format.
...
func (s *Server) Start() {
	...
	//[VIP functionality] Start monitoring if needed
	...
	//[VIP cluster config if it did] Start up routing as well if needed.
...
// [VIP] core functionality: AcceptLoop is exported for easier testing.
...
func (s *Server) startMonitoring(secure bool) error {
	...
	if secure {
		...
		//fmt.Println("hp port", httpListener.Addr().String(), ", original http port ", port)
	}
...
func (s *Server) createClient(conn net.Conn) *client {
	...
	// 1 Set the Ping timer, heartbeat can be sure that the connection is available.
	c.setPingTimer()

	//2  Spin up the read loop.
