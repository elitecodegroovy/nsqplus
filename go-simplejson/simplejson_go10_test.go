...
func TestNewFromReader1(t *testing.T) {
...
func TestSimplejsonGo10(t *testing.T) {
	...
	//
	mm := js.Get("test").Get("arraywithsubs").GetIndex(0).MustMap()
	fmt.Println("type: ", fmt.Sprintf("type %v", mm["subkeyone"]))
	//assert.Equal(t, true, mm["subkeyone"] == float64(1) )
	//assert.Equal(t, mm, map[string]interface{}{"subkeyone": float64(1)})

	assert.Equal(t, true, js.Get("test").Get("bignum").MustInt64() == 8000000000 )
}
