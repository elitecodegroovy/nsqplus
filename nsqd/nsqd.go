...
func New(opts *Options) *NSQD {
	...
	//dl does nothing if runtime environment is windows.
	n := &NSQD{
		startTime:            time.Now(),
...
func (n *NSQD) Main() {
	...
	//support HTTPS TCP listener if it is enabled.
	...
	//3 queue scan service
	n.waitGroup.Wrap(func() { n.queueScanLoop() })

	//4 pump service
	n.waitGroup.Wrap(func() { n.idPump() })

	//5 lookup loop service
	n.waitGroup.Wrap(func() { n.lookupLoop() })

	//6 stats service if it was configured
	if n.getOpts().StatsdAddress != "" {
		n.waitGroup.Wrap(func() { n.statsdLoop() })
	}
}

//load meta data if it exists.
// 1. atomic.Value converts to a struct `Options`
// 2. read file from file path.
func (n *NSQD) LoadMetadata() {
	// [Lock address]StoreInt32 atomically stores val into *addr. Then unlock.
